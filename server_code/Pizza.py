# This code is copied from Oven Pizza on May 16-24
...
class Pizza(object):
  ...
  def submit_pizza(self,action):
    ...
    if action == 'New':
      ...
      print(f'er: {er}')
      eventz_id = er[2]  # parsed from published record
      print(f'Published Record: {published_record}   Eventz Id: {eventz_id}')
      app_tables.pizzas.add_row(eventz_id=eventz_id,account=self.account,size=self.size, crust= self.crust, toppings= self.toppings, price= float(self.price), status=self.status)
    elif action == 'Update':
...
#       eventz_id = er[2]
#       print(f'Published Record: {published_record}   Eventz Id: {eventz_id}')
#       app_tables.pizzas.add_row(eventz_id=eventz_id,size=self.size, crust= self.crust, toppings= self.toppings, price= self.price)
